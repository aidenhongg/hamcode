...
if stage_at_or_after parse; then
    ...
    echo "=== [2f.5] strip leakage (Solution wrappers + complexity comments) ==="
    python pipeline/05b_strip_leakage.py \
        --in_path "${DATA_DIR}/interim/normalized/combined.jsonl" \
        --out "${DATA_DIR}/interim/stripped/combined.jsonl" \
        --audit_log "${DATA_DIR}/audit/strip_log.jsonl" \
        --fail_log "${DATA_DIR}/audit/strip_failures.jsonl"

    echo "=== [2g] dedupe + filter ==="
    python pipeline/06_dedupe_filter.py \
        --in_path "${DATA_DIR}/interim/stripped/combined.jsonl" \
