"""Single-experiment CLI: train one head with one seed.
...
def run(
    ...
    `language=None` (default) trains the universal head over all languages.
    `language=<lang>` slices the FeatureMatrix to that language only — used
    ...
    if language is None:
        ...
        if language not in per_lang:
            raise RuntimeError(
                f"language {language!r} dropped by build_per_language_splits "
                f"(see per_lang_meta.json)"
            )
        train, val, test = per_lang[language]

    # Class weight (computed on the slice — per-language imbalance differs)
    cw = None
    ...
    print(f"[train_head] fitting {head_name} (seed={seed}{lang_tag}) "
    ...
    test_met["split"] = "test"
    # Per-language test breakdown is only meaningful when the head was
    # trained universally; for a single-language head it would be a degenerate
    # one-bucket dict.
    if language is None and test.languages:
    ...
    lang_tag = f" lang={language}" if language is not None else ""
    print(f"[train_head]{lang_tag} test acc={test_met['accuracy']:.4f} "
...
def main() -> int:
    ...
    ap.add_argument("--out_dir", default=None,
                    help="Default: runs/heads/{head}-s{seed} (universal) or "
                         "runs/heads/per_lang/{lang}/{head}-s{seed} (per-language).")
    ap.add_argument("--class_weight", default="auto", choices=["auto", "none"])
    ap.add_argument("--language", default=None,
                    help="Train head on rows matching this language only "
                         "(e.g. python, java). Default: train universally.")
    args = ap.parse_args()
    ...
    if args.out_dir:
        ...
        out_dir = Path(
            f"runs/heads/per_lang/{args.language}/{args.head}-s{args.seed}"
        )
    else:
        out_dir = Path(f"runs/heads/{args.head}-s{args.seed}")
    run(
        head_name=args.head,
        seed=args.seed,
