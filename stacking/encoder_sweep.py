...
def _summarize_recipe(out_dir: Path) -> dict:
    """Recipe-level headline.

    Per-language sweep (default): the headline is the support-weighted mean
    of each language's best-head test macro-F1, read from
    PER_LANGUAGE_BEST.json. Higher = better.

    Universal sweep (legacy): fall back to picking the single best test row
    from SUMMARY.csv.
    """
    plb_path = out_dir / "PER_LANGUAGE_BEST.json"
    if plb_path.exists():
        try:
            plb = json.loads(plb_path.read_text(encoding="utf-8"))
        except Exception:
            plb = {}
        if plb:
            total_n = sum(int(r.get("n_test") or 0) for r in plb.values())
            if total_n > 0:
                weighted_f1 = sum(
                    int(r.get("n_test") or 0) * float(r.get("macro_f1") or 0.0)
                    for r in plb.values()
                ) / total_n
                weighted_acc = sum(
                    int(r.get("n_test") or 0) * float(r.get("test_acc") or 0.0)
                    for r in plb.values()
                ) / total_n
                # Best language for the small "headline head" column
                best_lang = max(
                    plb.items(),
                    key=lambda kv: float(kv[1].get("macro_f1") or 0.0),
                )
                return {
                    "status": "ok",
                    "mode": "per_language",
                    ...
                    "best_head": best_lang[1].get("head"),
                    "best_seed": best_lang[1].get("seed"),
                    "best_language": best_lang[0],
                    "test_macro_f1": weighted_f1,
                    "test_accuracy": weighted_acc,
                    "val_macro_f1": float("nan"),  # not available cross-lang
                }
...
def _write_encoder_summary(rows: list[dict], path: Path) -> None:
    ...
    lines = [
        ...
        "",
        "Recipes are scored by `test_macro_f1` — under per-language mode this "
        "is the support-weighted mean of each language's best-head test "
        "macro-F1 (read from `<recipe>/PER_LANGUAGE_BEST.json`); under the "
        "legacy universal mode it is the single best universal head's test "
        "macro-F1. The `mode` column reports which is in effect for each row.",
        "",
        "| recipe | mode | top head | seed | top lang | test F1 | test acc | wallclock | status |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    for r in rows:
        wallclock_h = r.get("wallclock_s", 0.0) / 3600
        if isinstance(r.get("test_macro_f1"), float):
            lines.append(
                f"| `{r['name']}` | {r.get('mode','-')} | "
                f"{r.get('best_head','-')} | {r.get('best_seed','-')} | "
                f"{r.get('best_language','-')} | "
                f"{r.get('test_macro_f1', float('nan')):.4f} | "
                f"{r.get('test_accuracy', float('nan')):.4f} | "
                f"{wallclock_h:.2f} h | {r.get('status','-')} |"
            )
        else:
            lines.append(
                f"| `{r['name']}` | - | - | - | - | - | - | "
