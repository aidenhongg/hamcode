...
# Import lazily so multiprocessing children can import independently.
...
def main() -> int:
    ...
    print(f"[prewarm] parquet={path}  task={task}  samples={len(payloads)}  workers={args.workers}",
          flush=True)
    t0 = time.time()
    n_built = 0
    n_cached = 0
    with mp.Pool(
        ...
        for built in tqdm(pool.imap_unordered(worker, payloads, chunksize=16),
    ...
    print(f"[prewarm] done in {dt:.1f}s  new={n_built}  already_cached={n_cached}  "
          f"rate={len(payloads)/max(0.1, dt):.1f}/s", flush=True)
    return 0
