...
def test_t9_retired_modules_unimportable() -> None:
    """Phase 1 deleted hp_search and train_heads_v2; importing them must error."""
    import importlib

    # stacking.hp_search lives at stacking/hp_search.py — deleted.
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("stacking.hp_search")

    # scripts/train_heads_v2.py (a script, not a package module). Use the file
    # path; its absence means the script was deleted as planned.
    assert not (REPO / "scripts" / "train_heads_v2.py").exists()


def test_t10_no_v2_variants_in_dataset() -> None:
    """The Variant Literal and VARIANT_DESCRIPTIONS table were retired."""
