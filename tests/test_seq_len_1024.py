"""seq=1024 + attention_window=1024 invariants.
...
def test_point_yaml_seq_1024_batch_8():
...
def test_lora_yaml_seq_1024_batch_8():
...
def test_train_config_default_seq_1024():
...
def test_lora_config_default_seq_1024():
...
def test_onnx_max_seq_len_1024():
    ...
    assert "1024 <= 1024" in src2


def test_model_default_attention_window_1024():
    """LongCoderClassifier picks up the pretrained-matching attention_window."""
    from model import DEFAULT_ATTENTION_WINDOW
    assert DEFAULT_ATTENTION_WINDOW == 1024
...
def test_build_point_inputs_shape_at_1024():
    from transformers import AutoTokenizer
    ...
    bundle = build_point_inputs(
        code, tok, max_seq_len=1024, bridge_stride=128, language="python",
    )
    assert bundle.input_ids.shape == (1024,)
    assert bundle.attention_mask.shape == (1024,)
    assert bundle.global_attention_mask.shape == (1024,)
    assert bundle.token_type_ids.shape == (1024,)
...
def test_longcoder_forward_at_seq_1024_no_oom():
    """Single-batch CPU forward at seq=1024. Slow but proves the attention
    layers don't blow up at this seq_len."""
    import torch
    from data import build_point_inputs
    from model import build_model
    from transformers import AutoTokenizer
